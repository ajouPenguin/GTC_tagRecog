__author__ = 'will'
from py3_simulation import vrep
import numpy as np
import cv2
import time
...
class RobotInterface():
    ...
    def setup(self):
        if self.clientID != -1:
            ...
            data = dict(list(zip(array, handles)))
            self.camera = [value for key, value in list(data.items()) if "Vision_sensor" in key][0]
            self.target = [value for key, value in list(data.items()) if "Quadricopter_target" == key][0]
